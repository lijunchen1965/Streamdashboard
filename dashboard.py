...
if fl is not None:
  ...
  #  df = pd.read_csv(filename, encoding = "ISO-8859-1")
    df = pd.read_csv(fl, encoding="ISO-8859-1")  # Use `fl` instead of `filename`
else:
    #os.chdir(r"c:\Python_Learning\Streamdashboard")
    #df = pd.read_csv("Superstore.csv", encoding = "ISO-8859-1")
    current_dir =os.getcwd()  # Get the current working directory
    df = pd.read_csv(os.path.join(current_dir, "Superstore.csv"), encoding="ISO-8859-1") # Load default file
...
# filter the data by region, state, and city
...
# Ensure month_year is a datetime-like object for correct sorting
linechart = filtered_df.groupby("month_year")["Sales"].sum().reset_index()
# Convert month_year to string for display but keep the correct order
linechart["month_year_str"] = linechart["month_year"].dt.strftime("%Y : %b")
fig2 = px.line(linechart, x = "month_year_str", y = "Sales", labels = {"Sales": "Amount"},height=500, width=1000,template='gridon')
...
# Create a treemap based on Region, category and subcategory
...
data1.update_layout(
                    title=dict(text="Sales and Profits Scatter Plot", font = dict(size=20)), 
